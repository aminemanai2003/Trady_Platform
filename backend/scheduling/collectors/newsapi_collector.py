...
def _is_relevant_forex_article(text: str, currencies: list[str]) -> bool:
    text_lower = text.lower()
    relevance_terms = [
        "forex", "currency", "exchange rate", "central bank", "interest rate",
        "inflation", "federal reserve", "european central bank", "bank of japan",
        "bank of england", "swiss national bank", "eur/usd", "usd/jpy",
        "gbp/usd", "usd/chf", "dollar index", "treasury yield", "bond yield",
        "rate cut", "rate hike", "monetary policy", "economic slowdown",
        "economic growth", "recession", "gold", "oil prices",
    ]
...
def collect_newsapi(max_articles: int = 50) -> dict:
    ...
    try:
        ...
        for art in articles:
            ...
            currencies = _detect_currencies(full_text)
            if not _is_relevant_forex_article(full_text, currencies):
                skipped += 1
                continue
            if not currencies:
                currencies = ["FX"]
            ...
            if existing is None:
                existing = NewsArticle.objects.filter(title=title, published_at=published_at).first()
