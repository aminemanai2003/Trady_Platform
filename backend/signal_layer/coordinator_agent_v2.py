...
class CoordinatorAgentV2:
    ...
    def _build_graph(self):
        ...
        def node_compute_weights(state: CoordinatorState) -> dict:
            updated_weights, weight_metadata = self._calculate_dynamic_weights(
                state["agent_signals"]
            )
        ...
        def node_aggregate_vote(state: CoordinatorState) -> dict:
            final_signal, confidence, weighted_score, effective_weights = self._aggregate_signals(
                state["agent_signals"],
                state["updated_weights"],
                state["regime"],
                state["weight_metadata"].get("evidence_coverage", 1.0),
            )
        ...
        def node_generate_explanation(state: CoordinatorState) -> dict:
            explanation = self._generate_explanation_text(
                ...
                state["effective_weights"], state["conflicts"]
            )
            deterministic_reason = self._generate_deterministic_reason(
                state["final_signal"], state["agent_signals"], state["effective_weights"]
            )
        ...
        def node_build_result(state: CoordinatorState) -> dict:
            result = {
                ...
                "weights_used":          state["effective_weights"],
                "weight_metadata":       state["weight_metadata"],
    ...
    def _calculate_dynamic_weights(self, agent_signals: Dict) -> tuple:
        """
        ...
        performance_weights = self.agent_weights.copy()
        method = "default_priors"
        if len(eligible) == len(self.agent_weights):
            # Use bounded positive scores and only a 20% performance tilt.
            # This prevents a small noisy sample from dominating the ensemble.
            adjusted = {
                name: max(min(perf.get("sharpe_ratio", 0.0), 2.0) + 2.0, 0.1)
                for name, perf in eligible.items()
            }
            total = sum(adjusted.values())
            learned = {name: value / total for name, value in adjusted.items()}
            performance_weights = {
                name: 0.8 * self.agent_weights[name] + 0.2 * learned[name]
                for name in self.agent_weights
            }
            method = "sample_gated_performance_tilt"

        # Evidence quality is separate from directional confidence. Missing or
        # partial source coverage reduces voting power before normalization.
        quality_adjusted = {
            name: performance_weights[name]
            * max(0.0, min(float(agent_signals[name].get("data_quality", 1.0)), 1.0))
            for name in self.agent_weights
        }
        quality_total = sum(quality_adjusted.values())
        if quality_total > 0:
            final_weights = {
                name: value / quality_total
                for name, value in quality_adjusted.items()
            }
        else:
            final_weights = self.agent_weights.copy()
            method = "default_priors_no_evidence"

        metadata = {
            "method": method,
            "minimum_outcomes_for_learning": 10,
            "evidence_coverage": round(quality_total, 4),
            "outcome_samples": {
                name: int(perf.get("trade_count", 0))
                for name, perf in performances.items()
            },
            "data_quality": {
                name: round(float(agent_signals[name].get("data_quality", 1.0)), 4)
                for name in self.agent_weights
            },
        }
    ...
    def _aggregate_signals(
        ...
        if regime == 'trending':
            ...
            # Keep relative weights unchanged. Confidence is reduced later;
            # scaling every weight equally before normalization has no effect.
            pass
        ...
        return final_signal, avg_confidence, float(weighted_signal_sum), regime_weights
    ...
    def _validate_with_correlations(self, symbol: str, signal: int, confidence: float) -> dict:
        ...
        if engine is None:
            return None
        try:
            correlation = engine.get_correlation_signals(symbol)
            matrix = correlation.get("correlation_matrix", {}) if correlation else {}
            momentum = correlation.get("pair_momentum", {}) if correlation else {}
            if signal == 0 or not matrix or not momentum:
                return None
            ...
            target_row = matrix.get(symbol, {})
            for partner, corr_value in target_row.items():
                if partner == symbol:
                    continue
                if abs(corr_value) < 0.3:
                    continue
                partner_momentum = float(momentum.get(partner, 0.0))
                if partner_momentum == 0:
                    continue
                implied_target_direction = int(
                    np.sign(partner_momentum) * np.sign(corr_value)
                )
                is_aligned = implied_target_direction == int(np.sign(signal))
                details.append({
                    'partner': partner,
                    'correlation': round(corr_value, 3),
                    'partner_momentum': round(partner_momentum, 6),
                    'implied_target_signal': implied_target_direction,
                    'alignment': 'supporting' if is_aligned else 'conflicting',
                })
                if is_aligned:
                    aligned_count += 1
                else:
            ...
            # Apply a deliberately small adjustment because correlation is
            # unstable and is confirmation evidence, not an independent vote.
            if aligned_count > conflicting_count:
                adjustment = min(1.08, 1.0 + 0.03 * aligned_count)
            elif conflicting_count > aligned_count:
                adjustment = max(0.85, 1.0 - 0.05 * conflicting_count)
            else:
                adjustment = 1.0
