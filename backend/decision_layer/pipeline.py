...
class TradingDecisionPipeline:
    ...
    def execute(
        ...
        try:
            ...
            # The structured language review is explanatory metadata only.
            # It cannot alter direction, confidence, or any safety decision.
            
            # Respect the actuarial layer's own minimum setup requirements.
            if actuarial_scores.get('verdict') != 'TRADE':
                ev_pips = actuarial_scores.get('expected_value_pips', 0)
                probability_win = actuarial_scores.get('probability_win', 0)
                logger.warning("Early rejection: setup below expectancy threshold")
                return self._build_rejection_response(
                    ...
                    actuarial_scores,
                    {
                        'verdict': 'REJECT',
                        'reasoning': (
                            f"Setup below expectancy threshold: EV {ev_pips:.1f} pips, "
                            f"estimated win likelihood {probability_win:.1%}"
                        ),
                        'latency_ms': 0,
                        'rejection_criteria': ['insufficient_expectancy'],
                        'from_cache': False
                    },
                    {
                        'approved': False,
                        'reason': 'Skipped due to early rejection',
                        'violations': ['insufficient_expectancy']
                    },
                    market_context,
                    tool_judge_output
                )
