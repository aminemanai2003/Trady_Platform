...
class TutorDocument(models.Model):
    ...
    file_type   = models.CharField(max_length=20)   # pdf, txt, png, mp3, ...
    modality    = models.CharField(max_length=20, default="text", db_index=True)
    topic       = models.CharField(max_length=200, blank=True)
    uploaded_at = models.DateTimeField(auto_now_add=True)
    chunk_count = models.IntegerField(default=0)
    extraction_metadata_json = models.TextField(blank=True, default="{}")
...
class DocumentChunk(models.Model):
    """
    A text-grounded evidence chunk with its embedding stored as JSON.
    """
    id            = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
