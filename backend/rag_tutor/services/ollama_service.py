...
7. Ignore excerpts that do not directly answer the question.
8. For image sources, treat OCR TEXT as authoritative. Treat VISUAL DESCRIPTION as
   lower confidence and never let it override visible OCR text.
9. Never move a number or label from one section to another. If a value is tied to
   AMBER, do not use it for RED; if it is tied to GREEN, do not use it for AMBER.
10. If a risk matrix says risk allocation is 0%, explain it as no trade/risk budget
    allocated. Do not describe it as "no risk involved."
...
Provide a clear educational explanation based solely on the directly relevant document context above.\
"""
