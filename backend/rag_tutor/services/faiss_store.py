...
# ── Module-level cache ─────────────────────────────────────────────────────────
# {(owner_email, dim): (index | None, meta: list[dict])}
# None means the user has no chunks; "MISSING" sentinel means not yet built.
_indexes: Dict[Tuple[str, int], Optional[Tuple]] = {}
...
def build_user_index(owner_email: str, target_dim: int) -> None:
    """
    ...
    if faiss is None:
        ...
        with _get_user_lock(owner_email):
            _indexes[(owner_email, target_dim)] = None
        return
    ...
    timestamp_starts: List[Optional[float]] = []
    timestamp_ends: List[Optional[float]] = []
    ...
    for chunk in qs:
        try:
            ...
            modalities.append(chunk.modality or "text")
    ...
    meta = [
        ...
        for t, f, m, p, s, e in zip(texts, filenames, modalities, page_numbers, timestamp_starts, timestamp_ends)
    ]

    with _get_user_lock(owner_email):
        _indexes[(owner_email, target_dim)] = (index, meta)
...
def search_user_index(
    ...
    cache_key = (owner_email, target_dim)

    # Lazy build
    if cache_key not in _indexes:
        build_user_index(owner_email, target_dim)

    entry = _indexes.get(cache_key)
    if entry is None:
        return []
    ...
    for score, idx in zip(scores[0], indices[0]):
        ...
        results.append({
            ...
            "modality":          meta[idx]["modality"],
            "page_number":       meta[idx]["page_number"],
            "timestamp_start":   meta[idx]["timestamp_start"],
            "timestamp_end":     meta[idx]["timestamp_end"],
        })
