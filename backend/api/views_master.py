...
class MasterSignalView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if auto_paper_trade and response_data.get("decision") == "APPROVED":
                paper_position = self._open_paper_trade(response_data, pair)

            # ── Log every signal so /reports has a row regardless of decision ─
            self._log_signal(response_data, pair, paper_position)
    ...
    def _format_response(self, result: dict, pair: str) -> dict:
        ...
        base: dict = {
            ...
            "coordinator": {
                ...
                "weight_metadata": coord.get("weight_metadata", {}),
                "cross_pair_validation": coord.get("cross_pair_validation"),
            },
            "judge": {
            ...
            "actuarial": {
                ...
                "historical_sample_size": actuarial.get("historical_sample_size", 0),
                "probability_basis": actuarial.get("probability_basis", "unknown"),
    ...
    def _open_paper_trade(self, response_data: dict, pair: str):
        """Auto-open a PaperPosition when pipeline approves a trade. Returns the
        created PaperPosition if available (for SignalLog linkage), or None."""
        try:
    ...
    def _log_signal(self, response_data: dict, pair: str, paper_position=None):
        """
        Persist a SignalLog row for every Generate Signal call. The Reports
        page counts these regardless of decision, so a user clicking
        "Generate Signal" always sees a bump in their signal count, even
        if the pipeline rejected or blocked the result.
        """
        try:
            from paper_trading.models import SignalLog

            signal = response_data.get("signal") or {}
            coord = response_data.get("coordinator") or {}
            rejection = response_data.get("rejection") or {}

            SignalLog.objects.create(
                pair=pair,
                direction=str(signal.get("direction") or "HOLD")[:8],
                confidence=float(signal.get("confidence") or 0.0),
                decision=str(response_data.get("decision") or "REJECTED")[:20],
                market_regime=str(coord.get("market_regime") or "")[:32],
                rejection_reason=str(rejection.get("reason") or "")[:255],
                snapshot=response_data,
                paper_position=paper_position,
            )
        except Exception as exc:
            # Logging must never break signal generation
            logger.debug(f"SignalLog write skipped: {exc}")
