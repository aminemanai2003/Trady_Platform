...
class TimeSeriesLoader:
    ...
    def latest_timestamp(
        ...
        source_timeframe = "1h" if timeframe in self.TIMEFRAMES and timeframe != "1h" else timeframe
        timeframe_values = self._timeframe_values(source_timeframe)
        ...
        # Compare with the configured relational store.
        relational_ts = self._relational_latest_timestamp(timeframe, symbols)
        if influx_ts is None and relational_ts is None:
            return None
        if influx_ts is None:
            return relational_ts
        if relational_ts is None:
            return influx_ts
        return max(influx_ts, relational_ts)

    def _relational_latest_timestamp(
        self,
        timeframe: str = "1h",
        symbols: Optional[List[str]] = None,
    ) -> Optional[datetime]:
        """Read latest OHLCV timestamp from the configured Django database."""
        try:
    ...
    def load_ohlcv(
        ...
        try:
            ...
            _log.getLogger(__name__).warning(f"InfluxDB unavailable ({_influx_err}), using relational fallback")
        ...
        if result is None or (isinstance(result, pd.DataFrame) and result.empty) or (isinstance(result, list) and not result):
            ...
            except Exception as _relational_err:
                import logging as _log2
                _log2.getLogger(__name__).warning(f"Relational OHLCV fallback failed: {_relational_err}")
            if result is None:
        ...
        if '_time' in result.columns:
            ...
            if not required_cols.issubset(set(result.columns)):
                return pd.DataFrame(columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
            df = result[['timestamp', 'open', 'high', 'low', 'close', 'volume']].copy()
