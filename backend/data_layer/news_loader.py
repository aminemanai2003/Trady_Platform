...
CURRENCY_TERMS = {
    ...
    'USD': ('usd', 'dollar', 'fed', 'fomc', 'united states'),
    'JPY': ('jpy', 'yen', 'boj', 'japan'),
    'GBP': ('gbp', 'pound', 'sterling', 'boe', 'united kingdom'),
...
class NewsLoader:
        ...
        try:
            ...
            if currencies:
                ...
                filtered = [
                    ...
                    if self._row_matches_currencies(row, currency_set)
                ][:limit]
    ...
    @staticmethod
    def _row_matches_currencies(row: dict, currency_set: set) -> bool:
        tagged = row.get('currencies')
        if isinstance(tagged, list):
            normalized = {str(currency).upper() for currency in tagged}
            if currency_set & normalized:
                return True
