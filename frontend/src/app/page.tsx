...
const dsos = [
  ...
  { code: "DSO1.3", label: "Sentiment Agent",   color: "#f59e0b", desc: "News and market sentiment scoring by currency pair" },
  ...
  { code: "Reports", label: "Signal Reports", color: "#67e8f9", desc: "Structured signal history with clear decision rationale" },
];

const team = [
  { name: "Ines Chtioui", role: "Project Lead", photo: "/team/ines-chtioui.jpg" },
  { name: "Amine Manai", role: "Project Manager", photo: "/team/amine-manai.jpg" },
  { name: "Mariem Fersi", role: "Solution Architect", photo: "/team/mariem-fersi.png" },
  { name: "Malek Chairat", role: "Data Scientist", photo: "/team/malek-chairat.jpg" },
  { name: "Maha Aloui", role: "Data Scientist", photo: "/team/maha-aloui.jpg" },
];
    ...
    <div className="min-h-screen bg-[#080d18] text-slate-100 overflow-hidden">
      ...
      <nav className="relative z-20 flex items-center justify-between px-8 py-5 border-b border-white/5 bg-[#080d18]/60 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/login"
            className="flex h-12 w-40 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03] text-sm font-semibold text-slate-200 transition-all hover:border-brand-blue-400/40 hover:bg-brand-blue-500/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue-500/40"
          >
            Login
          </Link>
          <Link
            href="/dashboard"
            className="flex h-12 w-40 items-center justify-center rounded-2xl border border-brand-blue-400/30 bg-gradient-to-r from-brand-blue-600 to-brand-green-600 text-sm font-bold text-white shadow-lg shadow-brand-blue-950/40 transition-all hover:scale-[1.02] hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue-500/40"
          >
            Dashboard -&gt;
          </Link>
        </div>
      ...
      <section className="relative z-10 px-6 py-12 border-t border-white/5">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <ShinyText text="The people building Trady" className="text-xs mb-6 block" color="#475569" shineColor="#8b5cf6" speed={6} />
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 items-stretch">
            {team.map((member, i) => (
              <motion.div
                ...
                className="overflow-hidden rounded-xl bg-white/[0.04] border border-white/10 text-left hover:border-white/20 transition-colors"
              >
                <div className="relative aspect-square bg-slate-900">
                  <Image src={member.photo} alt={member.name} fill sizes="(min-width: 1024px) 20vw, 50vw" className="object-cover" />
                </div>
                <div className="px-4 py-3 min-h-[74px]">
      ...
      <footer className="relative z-10 text-center py-6 border-t border-white/5 text-slate-600 text-xs">
        <ShinyText text="Trady - Your AI Trading Partner" color="#334155" shineColor="#6366f1" speed={8} />
      </footer>
    </div>
  );
}
