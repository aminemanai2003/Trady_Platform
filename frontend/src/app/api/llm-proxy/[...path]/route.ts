...
function inferActionFromContext(args: Record<string, unknown>): Record<string, unknown> {
  ...
  if (hints.includes("login") || hints.includes("navigate") || hints.includes("go to")) {
    const pages = ["dashboard", "trading", "agents", "reports", "monitoring", "backtesting", "strategy-tutor", "login", "register", "home"];
...
function normaliseAndCoerceAction(action: Record<string, unknown>): Record<string, unknown> {
  ...
  if (action["navigate_to_page"] && typeof action["navigate_to_page"] === "object") {
    ...
    if (typeof nav.page === "string" && !knownPages.has(nav.page)) {
      ...
      return { done: { text: "I couldn't find that page. Available pages: dashboard, trading, agents, backtesting, strategy-tutor, monitoring, login.", success: false } };
    }
  }
      ...
      try {
        ...
        if (
          ...
          const fallbackArgs = JSON.stringify({
            evaluation_previous_goal: "Assistant returned an empty response.",
        ...
        if (
          ...
          try {
            ...
            if (
              ...
              agentOutputArgs = {
                evaluation_previous_goal: "Assistant returned an unrecognized response.",
                memory: "",
            ...
            agentOutputArgs = {
              evaluation_previous_goal: "Assistant returned a plain text response.",
              memory: "",
        ...
        if (tc?.function?.arguments) {
          try {
            ...
            if (tc.function.name && tc.function.name !== "AgentOutput") {
              ...
              const wrapped = {
                evaluation_previous_goal: "Assistant returned a direct action.",
                memory: "",
