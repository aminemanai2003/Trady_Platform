...
export async function runOnboardingTour(): WorkflowResult {
  ...
  return agent.execute(`
    ...
    4. Navigate to the Backtesting page and explain the real returned metrics.
...
/**
 * Explain risk analysis via real backtesting and position sizing metrics.
 */
export async function explainRiskAnalysis(): WorkflowResult {
  const agent = await getAgentController();
  return agent.execute(
    `Navigate to the Backtesting page. ` +
      `Highlight the Max Drawdown, Sharpe Ratio, Win Rate, and Position Sizing areas. ` +
