...
export async function buildCustomTools(): Promise<Record<string, PageAgentTool | null>> {
  ...
  description:
    "Navigate to a named Trady page. Allowed targets: dashboard, trading, agents, reports, strategy-tutor, backtesting, billing, login, register, home.",
  inputSchema: z.object({
    page: z
      .string()
      .describe(
        "Target page name (e.g. 'agents', 'reports', 'strategy tutor')"
      ),
  }),
  execute: async ({ page }) => {
...
const click_element = tool({
  description:
    "Click a named UI element in the Trady app by its natural-language name. Examples: 'generate signal', 'export csv', 'logout', 'position sizing tab'. Resolves the element through the central registry.",
  inputSchema: z.object({
    element_name: z
      .string()
      .describe(
