...
export const api = {
    ...
    reportsSummary: (pair = "all", days = 90) =>
        fetcher<ReportSummaryResponse>(`/reports/summary/?pair=${encodeURIComponent(pair)}&days=${days}`),

    reportsExportUrl: (pair = "all", days = 90) =>
        `${API_BASE}/reports/export/?pair=${encodeURIComponent(pair)}&days=${days}`,
    ...
    recentNews: (classification = "all", limit = 50, search = "") =>
        fetcher<{
            articles: Array<{
                id: string;
                title: string;
            ...
            source_table: string;
            generated_at: string;
        }>(`/v2/news/recent/?classification=${encodeURIComponent(classification)}&limit=${limit}&search=${encodeURIComponent(search)}`),
    ...
    v2: {
        ...
        runBacktest: async (pair: string, lookbackBars: number): Promise<BacktestResponse> => {
            const response = await fetchWithTimeout(
                `${API_BASE}/v2/backtesting/run_backtest/`,
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    cache: "no-store",
                    body: JSON.stringify({ pair, lookback_bars: lookbackBars }),
                },
                180_000
            );
            if (!response.ok) {
                let message = `Backtest failed (HTTP ${response.status}).`;
                try {
                    const payload = await response.json();
        ...
        positionSizing: async (pair: string, capital: number, confidence: number): Promise<PositionSizingResponse> => {
            const response = await fetchWithTimeout(
                `${API_BASE}/v2/backtesting/position_sizing/`,
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    cache: "no-store",
                    body: JSON.stringify({ pair, capital, confidence }),
                },
                60_000
            );
            if (!response.ok) {
                let message = `Position sizing failed (HTTP ${response.status}).`;
                try {
                    const payload = await response.json();
                    message = payload?.error || message;
                } catch { /* ignore */ }
    ...
    dataIngest: {
        ...
        refreshMt5: () =>
            fetchWithTimeout(`${API_BASE}/v2/data/refresh_mt5/`, {
                method: "POST",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
            }, 10000).then((r) => r.json()),
        ...
        refreshGeopolitical: () =>
            fetchWithTimeout(`${API_BASE}/v2/data/refresh_geopolitical/`, {
                method: "POST",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
            }, 10000).then((r) => r.json()),
        ...
        refreshAll: () =>
            fetchWithTimeout(`${API_BASE}/v2/data/refresh_all/`, {
                method: "POST",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
            }, 10000).then((r) => r.json()),
