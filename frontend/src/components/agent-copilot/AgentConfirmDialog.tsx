...
export function AgentConfirmDialog() {
  ...
  return (
    /* Backdrop */
    <div
      className="fixed inset-0 z-[9999] flex items-center justify-center bg-foreground/35 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
      aria-labelledby="agent-confirm-title"
    >
      {/* Dialog */}
      <div className="relative w-full max-w-md mx-4 rounded-xl border border-border bg-popover text-popover-foreground shadow-2xl overflow-hidden">
        {/* Header strip */}
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          {inputDisplay && (
            <pre className="text-[11px] font-mono text-muted-foreground bg-muted rounded-lg p-3 overflow-x-auto max-h-32 whitespace-pre-wrap">
              {inputDisplay}
            </pre>
          )}
